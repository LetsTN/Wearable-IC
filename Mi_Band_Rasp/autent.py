...
from constants import UUIDS, ESTADO_AUTENT, TIPO_ALERTA, TIPO_FILA

class DelegacaoAutenticacao(DefaultDelegate):

	#Essa herda de DefaultDelegate para lidar com o processo de autenticação
	...
	def lidarNotificacao(self, hnd, data):

		# As possíveis notificações que devem ser lidadas
		...
		if hnd == self.device._char_auth.getHandle():
			if data[:3] == b'\x10\x01\x01':
				...
				self.device.state = ESTADO_AUTENT.FALHA_ENVIO_CHAVE
				...
				self.device.state = ESTADO_AUTENT.FALHA_REQU_NUMALE

			elif data[:3] == b'\x10\x03\x01':
				self.device.state = ESTADO_AUTENT.AUTENT_OK

			elif data[:3] == b'\x10\x03\x04':
				self.device.status = ESTADO_AUTENT.FALHA_ENCRIPTACAO_CHAVE
				self.device._send_key()

			else:
				self.device.state = ESTADO_AUTENT.AUTENT_FALHA

		elif hnd == self.device._char_heart_measure.getHandle():
			self.device.queue.put((TIPO_FILA.BATIMENTO, data))

		elif hnd == 0x38:
			# Not sure about this, need test
			if len(data) == 20 and struct.unpack('b', data[0])[0] == 1:
				self.device.queue.put((TIPO_FILA.RAW_ACEL, data))

			elif len(data) == 16:
				self.device.queue.put((TIPO_FILA.RAW_BATIMENTO, data))
...
class MiBand3(Peripheral):
	...
	_CHAVE = b'\x01\x23\x45\x67\x89\x01\x22\x23\x34\x45\x56\x67\x78\x89\x90\x02'
	_envia_chave_cmd = struct.pack('<18s', b'\x01\x08' + _CHAVE)
	_envia_rnd_cmd = struct.pack('<2s', b'\x02\x08')
	_envia_enc_key = struct.pack('<2s', b'\x03\x08')
	...
	def __init__(self, mac_address, timeout=0.5, debug=False):
		...
		self.svc_heart = self.getServiceByUUID(UUIDS.SERVICO_FREQ_CARD)

		self._char_auth = self.svc_2.getCharacteristics(UUIDS.CARACTERISTICA_AUTH)[0]
		self._desc_auth = self._char_auth.getDescriptors(forUUID=UUIDS.NOTIFICACAO_DESCRICAO)[0]

		self._char_heart_ctrl = self.svc_heart.getCharacteristics(UUIDS.CARACTERISTICA_FREQ_CARD_CONTROLE)[0]
		self._char_heart_measure = self.svc_heart.getCharacteristics(UUIDS.CARACTERISTICA_FREQ_CARD_MEDICAO)[0]
    ...
    def _parse_queue(self):
        ...
        while True:
            try:
                ...
                if self.heart_measure_callback and _type == TIPO_FILA.BATIMENTO:
                    self.heart_measure_callback(struct.unpack('bb', res[1])[1])

                elif self.heart_raw_callback and _type == TIPO_FILA.RAW_BATIMENTO:
                    self.heart_raw_callback(self._parse_raw_heart(res[1]))

                elif self.accel_raw_callback and _type == TIPO_FILA.RAW_ACEL:
    ...
    def initialize(self):
        ...
        while True:
            ...
            if self.state == ESTADO_AUTENT.AUTENT_OK:
    ...
    def get_revision(self):
        ...
        svc = self.getServiceByUUID(UUIDS.SERVICO_INFO_DISPOSITIVO)
    ...
    def get_hrdw_revision(self):
        ...
        svc = self.getServiceByUUID(UUIDS.SERVICO_INFO_DISPOSITIVO)
        char = svc.getCharacteristics(UUIDS.CARACTERISTICA_HRDW_REVISAO)[0]
        data = char.read()
    ...
    def set_encoding(self, encoding="en_US"):
        ...
        char = self.svc_1.getCharacteristics(UUIDS.CARACTERISTICA_CONFIG)[0]
    ...
    def set_heart_monitor_sleep_support(self, enabled=True, measure_minute_interval=1):
        ...
        char_m = self.svc_heart.getCharacteristics(UUIDS.CARACTERISTICA_FREQ_CARD_CONTROLE)[0]
        char_d = char_m.getDescriptors(forUUID=UUIDS.NOTIFICACAO_DESCRICAO)[0]
    ...
    def send_custom_alert(self, type):
        ...
        char = svc.getCharacteristics(UUIDS.CARACTERISTICA_CUSTOM_ALERTA)[0]
    ...
    def change_date(self):
        ...
        input('Data modificada, aperte enter para continuar')


    def dfuUpdate(self, fileName):

        print('Update Firmware/Resource')
        svc = self.getServiceByUUID(UUIDS.SERVICO_DFU_FIRMWARE)
        char = svc.getCharacteristics(UUIDS.CARACTERISTICA_DFU_FIRMWARE)[0]
        ...
        char1 = svc.getCharacteristics(UUIDS.CARACTERISTICA_DFU_FIRMWARE_ESCREVER)[0]
    ...
    def start_raw_data_realtime(self, heart_measure_callback=None, heart_raw_callback=None, accel_raw_callback=None):
            ...
            char_m = self.svc_heart.getCharacteristics(UUIDS.CARACTERISTICA_FREQ_CARD_MEDICAO)[0]
            char_d = char_m.getDescriptors(forUUID=UUIDS.NOTIFICACAO_DESCRICAO)[0]
            char_ctrl = self.svc_heart.getCharacteristics(UUIDS.CARACTERISTICA_FREQ_CARD_CONTROLE)[0]
    ...
    def stop_realtime(self):
            ...
            char_sens_d1 = char_sensor1.getDescriptors(forUUID=UUIDS.NOTIFICACAO_DESCRICAO)[0]
